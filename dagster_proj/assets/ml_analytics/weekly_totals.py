...
def sports_activity_dashboard(context: AssetExecutionContext, sport_type_totals):
    ...
    fig.add_trace(
        go.Pie(
            labels=sport_type_counts.index,
            values=sport_type_counts.values,
            name="Sport Types",
        ),
        row=2,
        col=1,
    ...
    fig.add_trace(
        go.Scatter(
            x=[df_weekly["week_start"].min(), df_weekly["week_start"].max()],
    ...
    fig.add_trace(
        go.Bar(
            x=df_weekly["week_start"], y=df_weekly["total_hours"], name="Total Hours"
        ),
        row=3,
        col=1,
    )
    ...
    fig.update_layout(
        height=1200, width=1200, title_text="Activity Totals (Last 8 Weeks)"
    )
    ...
    save_chart_path = Path(context.instance.storage_directory()).joinpath(
        "sports_activity_dashboard.html"
    )
