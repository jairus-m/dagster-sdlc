...
@dlt.resource(
    table_name="strava_activities", primary_key="id", write_disposition="merge"
)
def get_activities(
    header: dict, strava_activities_url: dlt.config.value, full_load: bool = False
) -> Generator:
    """
    ...
    while full_load or page_number == 1:
        ...
        response = requests.get(
            strava_activities_url, headers=header, params=param, timeout=100
        )
        data = response.json()
...
if __name__ == "__main__":
    pipeline = dlt.pipeline(
        pipeline_name="strava", destination="duckdb", dataset_name="activities"
    )
